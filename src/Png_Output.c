...
/*#define RGB(r, g, b) ((r) | ((g) << 8) | ((b) << 16) | 0xff)*/
#define RGB(r, g, b) ((r) | (g) | (b))

static const uint32_t png_crc32[256] = {
...
/**
 * \struct png_t
 * \brief a type representing a png image
 *
...
typedef struct {
    ...
    uint32_t crc;                /**< Current CRC32 checksum */
...
/* ------------------------------------------------------------------------ */
png_t *png_new(size_t width, size_t height) {
    png_t *png;
    if (SIZE_MAX / 4 / width < height) {
        /* ensure no type leads to an integer overflow */
        return NULL;
    }
    png = (png_t *) calloc(sizeof(png_t), 1);
    png->width = width;
    png->height = height;
...
/* ------------------------------------------------------------------------ */
static uint32_t png_swap32(uint32_t num) {
    return ((num >> 24) & 0xff) |
           ((num << 8) & 0xff0000) |
           ((num >> 8) & 0xff00) |
           ((num << 24) & 0xff000000);
...
/* ------------------------------------------------------------------------ */
static uint32_t png_crc(const unsigned char *data, size_t len, uint32_t crc) {
    size_t i;
    for (i = 0; i < len; i++) {
        crc = png_crc32[(crc ^ data[i]) & 255] ^ (crc >> 8);
    }
    return crc;
}

/* ------------------------------------------------------------------------ */
static void png_out_raw_write(png_t *png, const char *data, size_t len) {
    size_t i;
    for (i = 0; i < len; i++) {
...
/* ------------------------------------------------------------------------ */
static void png_out_raw_uint(png_t *png, uint32_t val) {
    *(uint32_t *) (png->out + png->out_pos) = val;
    png->out_pos += 4;
}

/* ------------------------------------------------------------------------ */
static void png_out_raw_uint16(png_t *png, uint16_t val) {
    *(uint16_t *) (png->out + png->out_pos) = val;
    png->out_pos += 2;
}

/* ------------------------------------------------------------------------ */
static void png_out_raw_uint8(png_t *png, uint8_t val) {
    *(uint8_t *) (png->out + png->out_pos) = val;
    png->out_pos++;
}

/* ------------------------------------------------------------------------ */
static void png_new_chunk(png_t *png, const char *name, size_t len) {
    png->crc = 0xffffffff;
    png_out_raw_uint(png, png_swap32((uint32_t) len));
    png->crc = png_crc((const unsigned char *) name, 4, png->crc);
    png_out_raw_write(png, name, 4);
}

/* ------------------------------------------------------------------------ */
static void png_end_chunk(png_t *png) {
    png_out_raw_uint(png, png_swap32(~png->crc));
}

/* ------------------------------------------------------------------------ */
static void png_out_uint32(png_t *png, uint32_t val) {
    png->crc = png_crc((const unsigned char *) &val, 4, png->crc);
    png_out_raw_uint(png, val);
}

/* ------------------------------------------------------------------------ */
static void png_out_uint16(png_t *png, uint16_t val) {
    png->crc = png_crc((const unsigned char *) &val, 2, png->crc);
    png_out_raw_uint16(png, val);
}

/* ------------------------------------------------------------------------ */
static void png_out_uint8(png_t *png, uint8_t val) {
    png->crc = png_crc((const unsigned char *) &val, 1, png->crc);
    png_out_raw_uint8(png, val);
}

/* ------------------------------------------------------------------------ */
static void png_out_write(png_t *png, const char *data, size_t len) {
    png->crc = png_crc((const unsigned char *) data, len, png->crc);
    png_out_raw_write(png, data, len);
}

/* ------------------------------------------------------------------------ */
static void png_out_write_adler(png_t *png, unsigned char data) {
...
/* ------------------------------------------------------------------------ */
static void png_pixel_header(png_t *png, size_t offset, size_t bpl) {
    if (offset > bpl) {
        /* not the last line */
...
char *png_get_data(png_t *png, size_t *len) {
    ...
    png_out_uint8(png, (uint8_t) PNG_RGB);
...
/* ------------------------------------------------------------------------ */
int png_save(png_t *png, const char *filename) {
    size_t len;
    FILE* f;
...
int PNG_output_file(const Image *I, const char *output_path){
    ...
    for (row = 0; row < I->h; row++) {
        for (col = 0; col < I->w; col++) {
			ImageGetPixel(I, &c, row, col);
			Color_to_uint32_t(&c, &value);
            /*png_set_pixel(png, x, y, RGB(c.r & 255, c.g & 255, c.b & 128));*/
            png_set_pixel(png, col, I->h - row - 1, value);
        }
    }
    png_save(png, output);
    png_destroy(png);
